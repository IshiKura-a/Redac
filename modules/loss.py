...
from modules.dataset import SubsetWrapper, RedacSubsetWrapper


class LossComputer:
    def __init__(self, criterion: nn.Module, is_robust: bool, dataset: SubsetWrapper, alpha: Optional[float] = None,
                 gamma: float = 0.1, adj: Optional[float] = None, min_var_weight: float = 0, step_size: float = 0.01,
...
class RedacLossComputer:
    def __init__(self, criterion: nn.Module, is_robust: bool, dataset: RedacSubsetWrapper,
                 alpha: Optional[float] = None, gamma: float = 0.1, adj: Optional[float] = None,
                 min_var_weight: float = 0, step_size: float = 0.01, normalize_loss=False, btl=False):
        self.criterion = criterion
        self.is_robust = is_robust
        self.gamma = gamma
        self.alpha = alpha
        ...
        self.n_clusters = dataset.n_clusters
        self.cluster_counts = dataset.cluster_counts.cuda()
        self.cluster_frac = self.cluster_counts / self.cluster_counts.sum()
        self.group_str = lambda x: f'Group {x}'

        if adj is not None:
            self.adj = torch.from_numpy(adj).float().cuda()
        else:
            self.adj = torch.zeros(self.n_clusters).float().cuda()

        if is_robust:
            assert alpha, 'alpha must be specified'

        # quantities maintained throughout training
        self.adv_probs = torch.ones(self.n_clusters).cuda() / self.n_clusters
        self.exp_avg_loss = torch.zeros(self.n_clusters).cuda()
        self.exp_avg_initialized = torch.zeros(self.n_clusters).byte().cuda()
    ...
    def compute_robust_loss(self, group_loss, group_count):
        adjusted_loss = group_loss
        if torch.all(self.adj > 0):
            adjusted_loss += self.adj / torch.sqrt(self.cluster_counts)
        if self.normalize_loss:
            adjusted_loss = adjusted_loss / (adjusted_loss.sum())
        self.adv_probs = self.adv_probs * torch.exp(self.step_size * adjusted_loss.data)
        self.adv_probs = self.adv_probs / (self.adv_probs.sum())
    ...
    def compute_robust_loss_btl(self, group_loss, group_count):
        adjusted_loss = self.exp_avg_loss + self.adj / torch.sqrt(self.cluster_counts)
        return self.compute_robust_loss_greedy(group_loss, adjusted_loss)

    def compute_robust_loss_greedy(self, group_loss, ref_loss):
        sorted_idx = ref_loss.sort(descending=True)[1]
        sorted_loss = group_loss[sorted_idx]
        sorted_frac = self.cluster_frac[sorted_idx]
    ...
    def compute_group_avg(self, losses, group_idx):
        # compute observed counts and mean loss for each group
        group_map = (group_idx == torch.arange(self.n_clusters).unsqueeze(1).long().cuda()).float()
        group_count = group_map.sum(1)
        group_denom = group_count + (group_count == 0).float()  # avoid nans
        group_loss = (group_map @ losses.view(-1)) / group_denom
        return group_loss, group_count
    ...
    def reset_stats(self):
        self.processed_data_counts = torch.zeros(self.n_clusters).cuda()
        self.update_data_counts = torch.zeros(self.n_clusters).cuda()
        self.update_batch_counts = torch.zeros(self.n_clusters).cuda()
        self.avg_group_loss = torch.zeros(self.n_clusters).cuda()
        self.avg_group_acc = torch.zeros(self.n_clusters).cuda()
        self.avg_per_sample_loss = 0.
        self.avg_actual_loss = 0.
        self.avg_acc = 0.
        self.batch_count = 0.
